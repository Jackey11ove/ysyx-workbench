...
#define immC() do { if(BITS(i, 31, 20)==0x341){*imm = 0;}else if(BITS(i, 31, 20)==0x300){*imm = 1;}else if(BITS(i, 31, 20)==0x342){*imm = 2;}else if(BITS(i, 31, 20)==0x305){*imm = 3;}} while(0)
...
//指令名称在代码中仅当注释使用, 不参与宏展开; 指令类型用于后续译码过程; 而指令执行操作则是通过C代码来模拟指令执行的真正行为
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int64_t)(src1) >> (BITS(imm,4,0)) );
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = SEXT((int32_t)BITS((int64_t)src1/(int64_t)src2,31,0),32));
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(rd) = SEXT((uint32_t)BITS(src1/src2,31,0),32));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = SEXT((int32_t)BITS((int64_t)src1%(int64_t)src2,31,0),32));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = (src1 + imm)&((uint64_t)(-1) << 2));
